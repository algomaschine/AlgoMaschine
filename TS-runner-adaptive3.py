...
def process_report_regex(regex):
	...
	for fda_low in pynum.arange(low, up, fda_step):	
		for fda_high in pynum.arange(fda_low, up, fda_step):
			for period in pynum.arange(min_period, max_period, step_period):
				# run a new process with each period
				com = "python ts-simple-adaptive3.py " + str(pFile) + " " + table + " " + str(period) + " " + str(fda_low) + " " + str(fda_high)
				cmd.put(com)
	...
	while (cmd.qsize()>0):
		...
		if (psutil.cpu_percent()<85 and ram_util()<80 ):
...
if __name__ == "__main__":
	...
	# python TS-runner-adaptive2.py 05k_7btc_5_optimized2 05k_7btc_5
	# todo: same with BTC
