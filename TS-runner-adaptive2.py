...
def process_report_regex(regex):
	...
	max_period = 100
	step_period = 2
	# that is the period to calculate FDA of a trading model
	# TODO: the next step would be also changing this period adaptively...
	...
	for fda_low in pynum.arange(low, up, fda_step):	
		for fda_high in [0.95]: # pynum.arange(fda_low, up, fda_step):
			for period in pynum.arange(min_period, max_period, step_period):
				# run a new process with each period
				com = "python ts-simple-adaptive2.py " + str(pFile) + " " + table + " " + str(period) + " " + str(fda_low) + " " + str(fda_high)
				#print(com)
				cmd.put(com)
	...
	while (cmd.qsize()>0):
		...
		if (psutil.cpu_percent()<90 and ram_util()<80 ):
...
if __name__ == "__main__":
	...
	# python TS-runner-adaptive2.py 05k_8btc_1_optimized2 05k_8btc_1   , another_pattern, another_pattern...
	#python TS-runner-adaptive2.py 20k_eu20_1 20k_eu20_1
