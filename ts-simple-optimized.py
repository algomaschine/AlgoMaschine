...
def write2mysql(ss):
	...
	for s in ss:
		...
		4 0.5,
		5 -2855.340000000004,
		6 -18.049999999997453
		'''
		sql = "INSERT INTO " + table + " (file, fda_min, fda_max, maj_threshold, fda_all_min, dd,  PnL) VALUES ('" + a[0] + "'," + a[1] + "," + a[2]+ "," +a[3] + "," + a[4] + "," + a[5] + "," + a[6]+ ")"
		print(sql)
...
def system_test(arrReports):
	...
	for fda_low in pynum.arange(low, up, fda_step):	
		#for fda_high in pynum.arange(1,1,1):	
		for fda_high in pynum.arange(fda_low, up, fda_step):
			for fda_all_t in pynum.arange(0.50, 0.52, 0.005):
